...
const ModelMalibu2025_1021 = ModelId("Malibu-v2.20251021")
const ModelMalibu2025_0710 = ModelId("Malibu-v2.20250710")
...
type Prompt struct {
	...
	MinP                   *float64       `json:"min_p,omitempty"`                      // Sets a minimum probability threshold relative to the most likely token. [0,1]
	ReturnTokensAsTokenIds *bool          `json:"return_tokens_as_token_ids,omitempty"` // Whether to return the generated tokens as token IDs instead of text. Defaults to false.
	Seed                   *bool          `json:"seed,omitempty"`                       // If specified, our system will make a best effort to sample deterministically, such that repeated requests with the same seed and parameters should return the same result. Determinism is not guaranteed.
	SkipSpecialTokens      *bool          `json:"skip_special_tokens,omitempty"`
	Stop                   []string       `json:"stop,omitempty"`   // An array of sequences where the API will stop generating further tokens.
	Stream                 *bool          `json:"stream,omitempty"` // If true, the response will be streamed as a series of events instead of a single JSON object.
	StreamOptions          *StreamOptions `json:"stream_options,omitempty"`
	Temperature            *float64       `json:"temperature,omitempty"` // What sampling temperature to use, between 0 and 2.
	Tools                  []Tool         `json:"tools,omitempty"`
	TopK                   *int64         `json:"top_k,omitempty"` //Limits the model to consider only the top K most likely tokens at each step.
	TopP                   *float64       `json:"top_p,omitempty"` //Limits the model to consider only the top K most likely tokens at each step.
...
type Message struct {
	...
	ToolCalls  []ToolCall `json:"tool_cals,omitempty"`
}
...
type ChatRequest struct {
	...
	// Valid values: "minimal" 5 only, "low", "medium", "high"
...
func (r ChatRequest) MarshalJSON() ([]byte, error) {
	...
	if isReasoningModel(string(r.Model)) {
...
func (c *Client) createChat(ctx context.Context, payload *ChatRequest) (*ChatCompletionResponse, error) {
	//if payload.Stream. != nil || payload.StreamingReasoningFunc != nil {
	//	payload.Stream = true
